    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (result.status === 'success') {
                ...
                // Update the frontend with the new evaluation data
                updateEvaluationStatus(result.total, result.evaluated, result.pending, department);
            } else {
                showStatusModal(result.status);
            }
        },
        error: function (err) {
            console.error(err);
...
function updateEvaluationStatus(total, evaluated, pending, department) {
    const departmentCard = document.querySelector(`#${department.replace(/\s/g, '')}Info`);
    departmentCard.querySelector('.badge-primary').textContent = `Total Employees: ${total}`;
    departmentCard.querySelector('.badge-success').textContent = `Evaluated: ${evaluated}`;
    departmentCard.querySelector('.badge-warning').textContent = `Pending: ${pending}`;
    // Update the progress bars
    const progressEvaluated = departmentCard.querySelector('.progress-bar.bg-success');
    progressEvaluated.style.width = `${(evaluated / total) * 100}%`;
    progressEvaluated.textContent = `Evaluated (${evaluated})`;
    const progressPending = departmentCard.querySelector('.progress-bar.bg-warning');
    progressPending.style.width = `${(pending / total) * 100}%`;
    progressPending.textContent = `Pending (${pending})`;
}
